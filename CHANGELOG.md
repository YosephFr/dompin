...
- **Recorded-session frame shortcut is explicit on Mac.** The manifest now declares `Command+Shift+1` for macOS and keeps `Ctrl+Shift+1` for Win/Linux, so Chrome registration and DOMPin instructions match the shortcut users actually need to press.
